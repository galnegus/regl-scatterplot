...
function findClosestIndices([x, y], { xMin, xMax, yMin, yMax }, N) {
...
function computePointValue(grid, point, xPos, yPos, bbox) {
  const [i1, i2, j1, j2] = findClosestIndices(point, bbox, grid.length);

  // NOTE: Grid uses indicing Grid[y][x] (y first) because of 'marchingsquares',
  // so that's why f21 = grid[j1][i2].
...
function computePointValues(grid, bbox, points) {
  ...
  return points.map(point => computePointValue(grid, point, xPos, yPos, bbox));
}
...
export default function findContour(grid, bbox, points, wingletsOptions) {
  const { xMin, xMax, yMin, yMax } = bbox;
  ...
  const isoValuesToTry = new Array(steps).fill().map((_, i) => minValue + i * dist);
  ...
  let contour = null;
  let backupContour = null;
  let nPoints = nPointsGreaterThanIsoValue(pointValues, isoValuesToTry[0]);
  let isoLine = MarchingSquares.isoLines(optimizedGrid, isoValuesToTry[0], { noFrame: true });

  for (let i = 1; i < steps; ++i) {
    const newNPoints = nPointsGreaterThanIsoValue(pointValues, isoValuesToTry[i]);
    const drop = 1 - newNPoints / nPoints;

    if (isoLine.length === 1) contour = isoLine[0];
    backupContour = isoLine[0];
    if (drop >= wingletsOptions.contourDropoff) break;

    nPoints = newNPoints;
    isoLine = MarchingSquares.isoLines(optimizedGrid, isoValuesToTry[i], { noFrame: true });
  }

  if (contour === null) {
    contour = backupContour;
    console.error("Can't find contour. Using best available one, (may look bad).");
    // throw new Error("Can't find contour.");
  }
